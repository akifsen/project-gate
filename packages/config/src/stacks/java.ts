...
export const javaAdapter: StackAdapter = {
  ...
  classify(file: string): FileClassification | null {
    ...
    if (/(^|\/)src\/test\//.test(file) || /Test\.(java|kt)$/.test(base) || /Tests\.(java|kt)$/.test(base)) {
  ...
  inspect(scan: ModuleScan): ModuleContribution | null {
    ...
    if (maven) {
      ...
      const ready = wrapped?.ready || toolOnPath("mvn");
      result.packageManagers.push(fact(wrapped ? "Maven Wrapper" : "Maven", wrapped ? commandName : "pom.xml", "java"));
      result.commands.push(command({ id: `${prefix}maven-test`, title: "Maven test", command: commandName, args: ["test"], group: "Test", invalidatesOn: patterns, ...cwd, ready }));
    ...
    if (gradle && !android) {
      ...
      const ready = wrapped?.ready || toolOnPath("gradle");
      result.packageManagers.push(fact(wrapped ? "Gradle Wrapper" : "Gradle", wrapped ? commandName : "build.gradle", "java"));
      result.commands.push(command({ id: `${prefix}gradle-test`, title: "Gradle test", command: commandName, args: ["test"], group: "Test", invalidatesOn: patterns, ...cwd, ready }));
    ...
    if (android && gradle) {
      ...
      result.commands.push(command({ id: `${prefix}android-test`, title: "Android unit test", command: commandName, args: ["test"], group: "Test", invalidatesOn: patterns, ...cwd, ready }));
    ...
    for (const tool of ["junit", "mockito", "assertj", "testcontainers", "archunit", "checkstyle", "spotbugs", "jacoco"]) {
      if (pom.toLowerCase().includes(tool) || gradleText.toLowerCase().includes(tool)) {
        result.capabilities.push({ name: tool, ready: true, adapter: "java", source: "build manifest", confidence: "observed" });
      }
    }
